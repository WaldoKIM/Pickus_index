...
include_once($_SERVER['DOCUMENT_ROOT'].'/estimate/my_estimate_match_list.php');

$rows = $config['cf_page_rows'];
$myEsiMath = new MyEstimateMatch($member);
$total_count=$myEsiMath->myEstimateMatchTotalCnt();
...
$result=$myEsiMath->myEstimateMatchList($from_record,$rows);
		...
		<div id="board">
			<div class="member">
				...
				for ($i = 0; $row = sql_fetch_array($result); $i++) {
					...
					$pro_cnt=$myEsiMath->myEstimateMatchPropose($no_estimate);
				?>
					<div class="req_list">
					<? if ($state == 9){
					...
					?>
					<?php echo get_estimate_state_match($state); ?></div>
						<h4 class="title_req">
