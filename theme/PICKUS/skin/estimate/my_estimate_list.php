...
include_once($_SERVER['DOCUMENT_ROOT'].'/estimate/my_estimate_list.php');
...
$myEstiList = new MyEstimateList($member);
$total_page=$myEstiList->myEstimateListTotalCnt();
...
<link rel="stylesheet" type="text/css" href="/theme/PICKUS/css/apms.css"/>
	...
	<div class="container">
		...
		<div id="board">
			<div class="member">
					...
					<?php
					$result=$myEstiList->myEstimateList($from_record, $rows);
    				...
    				<div class="req_list">
					<? if ($state == 9){
							echo "<div class='status_req_gopay'>";
						} else if ($state == 3){
						echo "<div class='status_req_confgoods'>";
						} else if ($state == 5){
							echo "<div class='status_req_delidone'>";
						} else{
						echo "<div class='status_req_user'>";
					}
					?><?php 
						if($e_type==2 && $state == 4){
						echo '철거중';
						...
						<h4 class="title_req"><?php
							...
							<div class="info_area">
								...
								<div class="ea_req">분류 : <?php echo get_etype($e_type);?></div>
							</div>
							<?php if($e_type == 3 || $e_type == 4 || $simple_yn == "1"){ ?>
...
$( document ).ready(function() {
	// setTimeout(function(){
	// 	webkit.messageHandlers.cordova_iab.postMessage(JSON.stringify({"action": "savelogin","loginid" : "<?= $member['mb_id']?>","passwd" : "<?= $member['mb_password']?>"}));
	// },300);
});
</script>
<script type="text/javascript">
// $( document ).ready(function() {
// 	setTimeout(function(){
// 		alert("loginid = " + "<?php echo $member['mb_id']?>");
// 		alert("passwd = " + "<?php echo $member['mb_password']?>");
// 		webkit.messageHandlers.cordova_iab.postMessage(JSON.stringify({"action": "savelogin","loginid" : "<?= $member['mb_id']?>","passwd" : "<?= $member['mb_password']?>"}));
// 	},300);
// });
	$(".mob_back").hide();
