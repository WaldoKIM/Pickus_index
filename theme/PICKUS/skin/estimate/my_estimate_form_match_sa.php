...
include_once($_SERVER['DOCUMENT_ROOT'].'/estimate/my_estimate_match_select.php');

$myEstMatch = new MyEstimateMatchSelect();

$mm=$myEstMatch->getMember($email);
...
$master=$myEstMatch->getEstmateMatch($no_estimate);
$shop=$myEstMatch->getShopOrder($no_estimate);
$info=$myEstMatch->getMatchInfo($no_estimate);

$centerCnt=$myEstMatch->getCenterCnt($no_estimate);
$propose_process=$myEstMatch->getProposeProcess($no_estimate);
$propose_select=$myEstMatch->getProposeSelect($no_estimate);
$propose_review=$myEstMatch->getProposeReview($no_estimate);
$propose_detail=$myEstMatch->getProposeDetail($no_estimate);



// if (count($master) == 0) {
// 	alert("해당 견적이 존재하지 않습니다.");
// }
...
<link rel="stylesheet" type="text/css" href="/theme/PICKUS/css/estimate/my_estimate_form_match_sa.css"/>
		...
		<div id="board">
			<div class="view">
				...
				<ul class="shop_list" id="proposeList">
					...
					if ($centerCnt > 0) {
						if ($state == "3" || $state == "4" || $state == "5" || $state == "8" || $state == "9") {
							$score_row=$myEstMatch->getSuccScoreRow($propose_success);
							...
							for ($i = 0; $row = sql_fetch_array($propose_process); $i++) {
								$score_row=$myEstMatch->getScoreRow($row);
								$score = $score_row['score'];
				...
				<ul class="shop_list" id="selectList">
					...
					?>
						<?php
						$request=$myEstMatch->getPartnerList($no_estimate,$master, $page);
					...
					?>
					<div id="page">
						<?php echo get_paging_shop_List(G5_IS_MOBILE ? 8: 8, $request["page"], $request["totalpage"], $request["state"] ,  $master['area1'],  $master['area2'], $e_type , $mm['mb_level'] , $no_estimate); ?>
					</div>
				</ul>
				<?php
				if ($state == "3" || $state == "4" || $state == "5" || $state == "8" || $state == "9") {
				...
				<?php
				$request_cnt=$myEstMatch->getContentCount($no_estimate);
				if ($request_cnt['cnt'] > 0) {
					$request_list=$myEstMatch->getContent($no_estimate);
