...
<link rel="stylesheet" type="text/css" href="/css/partner_estimate_form.css"/>
<?php
	...
	<div class="container">
		...
		<div id="board">
						...
						if($state == "1"){
							if($master["meet"]){
								if($e_type == "2"){
									echo "<ul class='row'>";
									echo "<li class='col-xs-6'>";
									echo "<a class='line_bg g_bg' href='javascript;'>방문중</a>";
									echo "</li>";
									echo "<li class='col-xs-6'>";
								...
								if($e_type == "2"){
									echo "<ul class='row'>";
									echo "<li class='col-xs-6'>";
									echo "<a class='line_bg l_bg white' href='javascript:doCancel();'>견적 취소</a>";
									echo "</li>";
				...
				<div class="web">
					...
					?>
						...
						<div class="pick_pay">
							...
							<p style="padding: 5px 0;">매입 견적:
								<? echo $success['price']. '원<br/>';?>
						</p> 
							<p style="padding: 5px 0;">수수료(VAT 포함): 
							<? 	$price_amt = $success['price'] * (10 / 100);
									$last_price = $price_amt + ($price_amt / 10);
									echo $last_price. '원<br/>';?>
						</p>
							...
							<!-- <p>(수수료와 VAT 포함가)</p> -->
							<p style="margin-top: 30px;">진행 완료 후 <span class="main_co">상기된 피커스계좌로</span><br> 최종 입금가 <span class="main_co">입금</span> 부탁 드립니다.</p>
						</div>
					<?php } 
						...
						<div class="pick_pay2">
							...
							<p style="padding: 5px 0;">폐기 견적:
								<? echo $success['price_minus']. '원<br/>';?>
						</p> 
							<p style="padding: 5px 0;">수수료(VAT 포함): 
							<? 	$price_amt = $success['price_minus'] * (10 / 100);
									$last_price = $price_amt + ($price_amt / 10);
									echo $last_price. '원<br/>';?>
						</p>
							

							<p style="padding: 5px 0;">최종 폐기 대금: 
								<span class="main_co" style="font-weight: bold;"><?php									
									$last_pricex = $success['price_minus'] - $last_price;
									 echo number_format(floor($last_pricex)) . '원<br/>';
									?>
								</span>
							</p>
							<p>(견적가에서 수수료 및 VAT를 제외한 금액)</p>
				...
				<?php
					if($propose_review){
						...
						if($propose_review['photo1']){
							echo "<th>".estimate_img_thumbnail($propose_review['photo1'], 500, 500)."</th>";
						}else{
								echo "<th>".estimate_img_thumbnail($photo['photo'], 500, 500)."</th>";
						}
						echo "<td>";
			...
			<div class="btn_wrap">
				<ul class="row">
					<li class="col-xs-12">
						<a class="dark_knight white" href="./partner_estimate_list.php?gubun=<?php echo $gubun; ?>&&page=<?php echo $page; ?>">목록으로 돌아가기</a>
...
<form name="frmcancel_del" id="frmcancel_del" action="<?php echo G5_URL; ?>/estimate/partner_estimate_form_update_cancel.php"  method="post" enctype="multipart/form-data" autocomplete="off" style="display: none;" onsubmit="doCancel();">
	<input type="hidden" id="idx"  name="idx" value="<?php echo $master['idx']; ?>">
	<input type="hidden" id="sub_idx" name="sub_idx" value="<?php echo $master['sub_idx']; ?>">
	...
	<div style="position: absolute; top: 50%; left: 50%; padding: 20px; background-color: #fff; border:1px solid #ddd; transform: translate(-50%, -50%);">
		...
		<input style="background-color: #ff1616; color: #fff; margin-top: 15px;" type="submit" name="" value="수거취소">
		<input style="background-color: #ff1616; color: #fff; margin-top: 15px;" type="button" name="popCancel" value="닫기" onclick="popupCancel()">
	</div>
</form>
