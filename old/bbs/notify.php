...
$sql_common = " from
					{$g5['notify_table'] }
				where
					email = '{$member['mb_email']}' and read_gb = '0' ";
				...
				<table>
					...
					<?php
	    				for ($i=0; $row=sql_fetch_array($result); $i++)
	    				{
	    					...
	    					echo "<td>";
							if($row['estimate_idx']  == '0'){
								echo "<a class='btn main_bg' href='javascript:doMoveNotify(\"".$row['idx']."\",\"".$row['noti_type']."\",\"".$row['estimate_idx']."\",\"".$row['category']."\")'>확인</a>";
							}else{
...
<form name="frmmove" action="<?php echo G5_URL; ?>/bbs/notify.update.php" method="post" enctype="multipart/form-data" autocomplete="off">
	...
	<input type="hidden" id="category1" name="category1" value="<?php echo G5_URL; ?>">
</form>
